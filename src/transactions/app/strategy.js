...
passport.use(
  ...
  new JwtStrategy(opts, async (jwt_payload, done) => {
    try {
      let user;
    //   user = await redis.json.get(jwt_payload._id.toString(), {
        //fetching the user from cache if exists
        // path: ".",
    //   });
    //   if (!user) {
        user = await User.findById(jwt_payload._id); //search for the user in primary database
        // if (user) await redis.json.set(user._id.toString(), ".", user); // if found cache the user in redis
    //   }
      if (user) {
